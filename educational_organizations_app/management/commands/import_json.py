                            ...
                            with open(image_path, 'rb') as f:
                                document, created = Document.objects.update_or_create(
                                    ...
                                    defaults={
                                        ...
                                        'image': File(f)  # Pass the file object directly
                                    }
                                )
                        else:
                            # If no image file or path is provided, handle other fields
                            document, created = Document.objects.update_or_create(
                                id=pk,
                                defaults={
                                    'type': fields.get('type'),
                                    ...
                                    'file_name': fields.get('file_name'),
                                    'file_name_system': fields.get('file_name_system'),
                                    # Set 'image' to None or keep the existing value if image_path is not provided
                                }
                            )
                        
                        self.stdout.write(self.style.SUCCESS(
                            f'Successfully imported document "{document.file_name}"'))
                    
                    except User.DoesNotExist:
                        raise CommandError(f'User with id {fields.get("user")} does not exist.')
                    except Exception as e:
